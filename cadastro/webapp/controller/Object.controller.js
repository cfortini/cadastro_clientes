sap.ui.define([
	...
	return BaseController.extend("cfortini.cadastro.controller.Object", {
		...
		onGravar2: function(){
			...
			if(!oModel.hasPendingChanges()){
				MessageToast.show('Não houve alteração dos dados!');
				return;
			}
		...
		onCancelar:function(){
			...
			if (!m.hasPendingChanges()){
				MessageToast.show('Não houve alterações');
				return;
			}
		...
		_onObjectMatched : function (oEvent) {
			...
			this.getModel().metadataLoaded().then( function() {
				var sObjectPath = this.getModel().createKey("dadosSet", {
					ID :  sObjectId
				});
